...
// Fetch products that are low in quantity
$query = "SELECT * FROM products WHERE quantity <= $quantity_threshold";
$result = mysqli_query($conn, $query);

$low_quantity_products = [];
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $low_quantity_products[] = $row;
    }
}
...
<head>
    ...
    <title>Low Quantity Alerts</title>
...
<div class="alert-container">
    <h2>Low Quantity Alerts</h2>
    <?php if (empty($low_quantity_products)) { ?>
        <p>No products are low in quantity.</p>
    <?php } else { ?>
        <table>
            <thead>
                <tr>
