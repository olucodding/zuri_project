...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_query($conn, $order_query)) {
        ...
        foreach ($cart_items as $product_id => $quantity) {
            $order_item_query = "INSERT INTO order_items (order_id, product_id, quantity, price) VALUES ('$order_id', '$product_id', '$quantity', '{$products[array_search($product_id, array_column($products, 'product_id'))]['price']}')";
        ...
        if ($payment_method == 'bank_transfer') {
            if (isset($_FILES['transfer_proof']) && $_FILES['transfer_proof']['error'] == UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($file_tmp, $file_path)) {
                    $sql = "INSERT INTO order_payment (order_id, payment_method, transfer_proof) VALUES ('$order_id', 'Bank Transfer', '$file_path')";
