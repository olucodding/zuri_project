...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "UPDATE products SET product_name = ?, description = ?, price = ?, category = ?, quantity = ?, status = ? $image_sql WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);

        if ($image_sql) {
            mysqli_stmt_bind_param($stmt, 'ssdssissi', $product_name, $description, $price, $category, $quantity, $status, $image, $product_id);
        } else {
            mysqli_stmt_bind_param($stmt, 'ssdssisi', $product_name, $description, $price, $category, $quantity, $status, $product_id);
        }
    ...
    <form action="edit_product.php?id=<?php echo $product_id; ?>" method="POST" enctype="multipart/form-data">
        ...
        <div>
            ...
            <select id="status" name="status" required>
                <option value="available" <?php echo $product['status'] == 'active' ? 'selected' : ''; ?>>Available</option>
